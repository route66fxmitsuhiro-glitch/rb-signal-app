...
いずれも UTC。**執行推奨時刻(日足の確定から90分後 = 夏 7:30 / 冬 8:30 JST)**を
狙っています。夏は1本目、冬は2本目が当たり、外れた方はウィンドウ外として数秒で
終了します。3本目は遅延・失敗時のフォールバックです。

> 2026-09-22 に変更。旧設定(20:45 / 21:45 / 23:30)は**日足が確定する前にも
> 発火しうる**うえ、通知がロールオーバー直後(1日でスプレッドが最も広がる瞬間、
> GBPJPYで20〜30pips)に届いていました。実測に基づき執行を90分後にずらしています。
